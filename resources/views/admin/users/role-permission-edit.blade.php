...
<div class="content-wrapper">
  ...
  <section class="content">
    <div class="container-fluid">
        ...
        @endif
        <form action="{{ route('UpdateRolePermissions') }}" method="POST">
            @csrf
            ...
            <div class="row">
                ...
                <div class="card card-primary">
                    ...
                    <h3 class="card-title text-center">{{ $role->name.' Permission '. $role->permissions->count() }}</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @if ($role->permissions->count() == 0)
                                <tr>
                                    <td align="center" colspan="5"><h2>{{ __('You have no permission') }}</h2></td>
                                </tr>
                            @else
                            @foreach ($role->getPermissionNames() as $permission)
                            <tr>
                                <td class="text-center">{{ $loop->index + 1 }}</td>
                                <td class="text-center"><input type="checkbox" name="permission[]" value="{{ $permission }}" {{ $role->getPermissionNames() ? "checked" : '' }}></td>
                                <td>{{ $permission }}</td>
                                <td class="text-center">{{ $role->created_at->format('d M Y') }}</td>
                                <td class="text-center">{{ $role->updated_at->format('d M Y') }}</td>
                            </tr>
                            @endforeach
                            @endif
